...
i2c8Bit::i2c8Bit(void){
    ...
    this->deviceAddress.push_back(0);
...
i2c8Bit::i2c8Bit(vector<unsigned char> dev_addr, std::string i2c_file_name,
...
int i2c8Bit::writeReg(unsigned char reg_addr, unsigned char data){
    ...
    messages[0].addr = deviceAddress[0];
    messages[0].flags = 0;
...
int i2c8Bit::writeAllReg(unsigned char reg_addr, unsigned long long data){
    ...
    { 
        buff[0] = reg_addr;
        buff[1] = (unsigned char)((data >> (i*8)) & 0xff);
   
        messages[0].addr = deviceAddress[i];
        messages[0].flags = 0;
        messages[0].len = sizeof(buff);
        messages[0].buf = buff;
...
int i2c8Bit::readReg(unsigned char reg_addr, unsigned char &data){
    ...
    inbuff = &data;
    messages[1].addr = deviceAddress[0];
