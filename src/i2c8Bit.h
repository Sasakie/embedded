...
class i2c8Bit 
{
    public:
        ...
        i2c8Bit(std::vector<unsigned char> dev_addr, std::string i2cfilename, 
                    unsigned int num_chips); 
        //over loaded constructor
        ~i2c8Bit(void); // destructor
        int writeReg(unsigned char reg_addr, unsigned char data);
        int writeAllReg(unsigned char reg_addr, unsigned long long data);
    ...
    private:
        ...
        std::vector<unsigned char> deviceAddress; // i2c device address
};
